...
automation_runs(id, tenant_id, automation_id, trigger_ref jsonb,
                ...
                trigger_variables jsonb NULL,   -- AU-25: o contexto do gatilho, nunca sobrescrito
...
| CL-02 | Autocomplete de endereço por CEP, com preenchimento manual quando a consulta falhar. **A consulta sai do nosso servidor, não do navegador** (`GET /v1/public/cep/:cep`): a CSP do front não lista o provedor em `connect-src`, e a chamada feita na tela vinha sendo bloqueada em silêncio — o `catch` a transformava em "CEP não encontrado", que é o mesmo que se vê quando o CEP não existe. Fazê-la no servidor também é o que permite oferecê-la na página pública de inscrição (IN-25) sem dar a um terceiro o IP de todo interessado, que é a objeção que manteve o captcha fora do escopo (SEC-20). Cache de processo, prazo de 4s, e o provedor fora do ar cai no preenchimento à mão — endereço nunca bloqueia nada. |
...
| AU-05 | **Ação de automação não dispara automação.** O gatilho nasce na borda HTTP e o motor chama o caso de uso direto — a classe inteira de "automação que se alimenta" não existe. O que segura a escala é a **fila**, não um teto que recusa: execução nenhuma é descartada, e o ritmo vem do lote de cada passada — o que não coube fica pendente e a passada seguinte pega. Somam-se um teto de passos por execução (fluxo torto não anda para sempre) e o interruptor, que **cancela o que está na fila**: desligar para de verdade, mesmo com centenas de execuções pendentes. |
...
| AU-16 | Os campos do contexto são **oferecidos em lista**, por gatilho, mais as variáveis que o próprio fluxo definiu — e o texto de mensagem tem inseridor de campo. Digitar `contato.fone` de memória não dá erro nenhum (AU-09 transforma em vazio), a mensagem sai sem o nome do cliente e ninguém descobre. O catálogo mora no domínio e é cobrado por teste da borda: o que o seletor promete, o gatilho entrega. |
| AU-17 | Gatilhos de **mensagem enviada** (a equipe respondendo pela caixa — o eco do provedor não conta, senão AU-05 cai por uma porta lateral), **inscrição cancelada** (com o motivo no contexto) e **de tempo em tempo**: a cada N minutos, horas ou dias, sem entidade por trás. O recorrente é varrido como o temporal, e o que impede o disparo múltiplo é a **fatia de tempo** virando chave de idempotência — sem guardar "quando disparou pela última vez", que se perderia num restauro. Piso de um minuto, o mesmo da espera e pela mesma razão: a varredura de rede é de sessenta segundos. |
| AU-18 | Bloco **para cada**: percorre uma lista do sistema — as entidades que já existem, com os campos de cada uma —, filtra por campo/operador/valor com os **mesmos operadores do bloco Se**, e **semeia uma execução por item**, cada uma com o contexto de um. Não itera dentro de uma execução: é isso que mantém o log respondendo "por que **este** cliente?" (AU-06), faz o teto de passos e as tentativas valerem por item, e evita um ciclo no grafo (AU-07). É mecanismo, não pergunta pronta: quem monta a pergunta é a equipe. A chave de idempotência é bloco + entidade + fatia de tempo. Um "para cada" por automação, e o que ele semeia entra na fila — nada é recusado (AU-05). |
| AU-19 | Bloco **buscar**: procura **um** item na mesma lista do "para cada", com os mesmos filtros, e separa o caminho em **achou** e **não achou**. O que ele acha entra no contexto da execução, então o "Se" e as ações adiante passam a enxergar campos que o gatilho não trouxe — é o que permite "o lead escreveu; se não existe cartão dele no funil, crie". O **valor do filtro aceita variável**: o campo se lê no item, o valor se lê no contexto da execução, e é assim que se procura pelo contato de quem acabou de escrever. |
| AU-20 | A busca traz **o primeiro** ou **todos**. "Todos" guarda a lista sob um nome, e quem a percorre é o bloco **para cada** — que deixou de buscar por conta própria. Separar buscar de iterar é o que permite olhar o resultado antes de agir: contar, condicionar, avisar a equipe se veio vazio. O nome da lista é validado ao salvar: apontar para uma lista que nenhuma busca guarda percorreria zero em silêncio, que é o pior defeito — o que parece funcionar. As **fichas de cliente** entram no catálogo, **sem CPF**: o contexto de automação vira texto de mensagem, e documento de identidade não passeia por aí. |
| AU-21 | **A porta para fora e para dentro.** Ação **chamar URL** (webhook de saída, ou HTTP genérico): só `https`, **nunca endereço de rede interna** — julgado antes e depois do DNS, porque host público pode resolver para IP interno e o alvo clássico é o metadado da nuvem, o banco em `10.x` ou a própria API em `localhost` —, prazo de dez segundos e resposta cortada, que vai para o log e, quando o bloco pede por `saveAs` (AU-23), também para o contexto. Gatilho **webhook recebido**: uma URL por gancho, autenticada pela **API key do tenant** com escopo próprio (`automation:trigger`), 401 uniforme e rate limit por chave, no mesmo desenho do webhook de inscrições (§5.7). O corpo entra em `webhook.corpo`, e só a automação daquele nome acorda. |

| AU-22 | **Funções dentro do texto.** O marcador aceita chamada nomeada além do caminho: `primeiroNome`, `nomeProprio`, `maiuscula`, `minuscula`, `padrao`, `data`, `diasAte`, `dinheiro`. São **poucas e nomeadas de propósito** — expressão que aceita qualquer coisa vira linguagem, e linguagem dentro de um campo de texto é código sem revisão e sem teste. A regra do AU-09 continua valendo por cima de tudo: o que não dá certo vira vazio, nunca o marcador cru na cara do cliente. `dinheiro` existe porque dinheiro é centavos no sistema inteiro, e "R$ 258000" numa mensagem seria o erro mais caro que um texto pode ter. A tela lista as funções onde o texto é escrito. |
| AU-23 | **Nó de código, e o retorno da ação virando variável.** Bloco **rodar código**: JavaScript sobre os dados do fluxo, isolado em `node:vm` — contexto sem protótipo, sem `require`/`process`/`fetch`/timer, geração de código desligada, dados entrando **serializados** e retorno saindo como texto JSON, prazo de 500ms e teto de 20 mil caracteres. Isso torna o escape difícil, **não impossível**: `vm` isola escopo, não processo; a guarda de verdade é que quem escreve o código já é owner ou admin do tenant, e no dia em que houver tenant não confiável a troca certa é `isolated-vm` ou processo separado. Junto vem o mecanismo que faltava para qualquer ação: a chave **`saveAs`** guarda o que a ação devolveu sob um nome, e o resto do fluxo passa a enxergar (`{{resposta.status}}`). Sem nome pedido, o retorno fica só no log. O código **não** passa pela troca de marcador: `if (x) {{ y }}` é bloco dentro de bloco em JavaScript. |
| AU-24 | **Caminho de erro por ação.** Toda ação tem duas saídas: a normal e a de **erro**. Ligar a de erro é dizer que aquela falha é prevista — o parceiro recusou, o cartão não estava mais lá —, e aí o motor **não retenta**: desvia, com o motivo em `erro.motivo`. Sem a saída ligada, nada muda: continua sendo tentativa e, no fim, falha (AU-11), que é o certo para provedor fora do ar. O erro fica no log dos dois jeitos — desviar não é esconder. |
| AU-25 | **Ensaio.** Percorrer o desenho com dados informados, sem ligar a automação: mostra por onde ela passaria e o que cada ação **receberia**, com os marcadores já trocados. Ação nenhuma é executada e nada é gravado — ensaio não vira execução e não entra no log (AU-06), senão o registro de "o que o sistema fez sozinho" passaria a ter dentro coisas que ele não fez. As **buscas rodam**, porque só leem, e é o que faz o ensaio responder à pergunta que interessa: com os dados de hoje, esta condição dá sim ou não? |
