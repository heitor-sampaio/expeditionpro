...
| AU-18 | Bloco de **busca**: lê uma lista (conversas paradas há N minutos, esperando o contato ou esperando a equipe) e **semeia uma execução por achado**, cada uma com o contexto de um. Não itera dentro de uma execução — é isso que mantém o log respondendo "por que **este** cliente?" (AU-06), faz o teto de passos e as tentativas valerem por item, e evita um ciclo no grafo (AU-07). A chave de idempotência é `bloco : entidade : fatia do próprio intervalo`, então uma busca que roda de cinco em cinco minutos não semeia a mesma conversa doze vezes por hora. Uma busca por automação, com teto de 25 por passada, e as execuções semeadas contam no teto de 20 por hora. |
