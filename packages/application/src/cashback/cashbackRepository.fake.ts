...
/** Fake in-memory do port de cashback. Excluído do build (`*.fake.ts`). */
export function fakeCashbackRepository(seed?: {
  config?: CashbackConfig;
  override?: CashbackOverride;
}): CashbackRepository & { rows: (CashbackEntryRecord & { tenantId: string })[] } {
