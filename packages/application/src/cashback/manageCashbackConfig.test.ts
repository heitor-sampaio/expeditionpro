...
describe('CB-02: leitura da config de cashback', () => {
  it('retorna a config desligada por padrão (módulo nasce off)', async () => {
    ...
    const config = await getCashbackConfig({ cashback }, ctxWith('admin'));
  ...
  it('é da equipe: contexto de cliente é recusado', async () => {
    ...
    await expect(getCashbackConfig({ cashback }, customerCtx)).rejects.toBeInstanceOf(
      ForbiddenError,
    );
  });
});
...
describe('CB-01/CB-02: atualização da config de cashback', () => {
  it('persiste a config e a leitura seguinte reflete', async () => {
    const cashback = fakeCashbackRepository();
    const saved = await updateCashbackConfig({ cashback }, ctxWith('owner'), VALID);
    expect(saved.enabled).toBe(true);
    expect(saved.value).toBe(5);
    const read = await getCashbackConfig({ cashback }, ctxWith('admin'));
  ...
  it('CB-01: percentual acima de 100 é recusado', async () => {
    const cashback = fakeCashbackRepository();
    await expect(
      updateCashbackConfig({ cashback }, ctxWith('admin'), {
        ...VALID,
        mode: 'percent',
        value: 150,
      }),
  ...
  it('CB-01: valor fixo negativo é recusado', async () => {
    const cashback = fakeCashbackRepository();
    await expect(
      updateCashbackConfig({ cashback }, ctxWith('admin'), { ...VALID, mode: 'fixed', value: -1 }),
    ).rejects.toBeInstanceOf(BusinessRuleError);
  });
  ...
  it('CB-06: teto de resgate fora de 0..100 é recusado', async () => {
    const cashback = fakeCashbackRepository();
    await expect(
      updateCashbackConfig({ cashback }, ctxWith('admin'), { ...VALID, maxRedemptionPct: 120 }),
    ).rejects.toBeInstanceOf(BusinessRuleError);
  });
  ...
  it('dias de liberação negativos são recusados', async () => {
    const cashback = fakeCashbackRepository();
    await expect(
      updateCashbackConfig({ cashback }, ctxWith('admin'), { ...VALID, releaseDays: -5 }),
    ).rejects.toBeInstanceOf(BusinessRuleError);
  });
  ...
  it('mexer em dinheiro da empresa exige owner ou admin: operator é recusado', async () => {
    const cashback = fakeCashbackRepository();
    await expect(
      updateCashbackConfig({ cashback }, ctxWith('operator'), VALID),
