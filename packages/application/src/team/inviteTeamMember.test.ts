...
describe('§3.7: convite de membro de equipe (Admin API + app_metadata)', () => {
  it('owner convida operator, gravando tenant_id do contexto e o papel no app_metadata', async () => {
    ...
    const result = await inviteTeamMember({ authAdmin }, ctxWith('owner'), {
  ...
  it('admin também pode convidar', async () => {
    const authAdmin = fakeAuthAdminGateway();
    await inviteTeamMember({ authAdmin }, ctxWith('admin'), { email: 'a@b.com', role: 'viewer' });
    expect(authAdmin.invites).toHaveLength(1);
  });
  ...
  it('operator não pode convidar (só owner/admin)', async () => {
    const authAdmin = fakeAuthAdminGateway();
    await expect(
      inviteTeamMember({ authAdmin }, ctxWith('operator'), { email: 'a@b.com', role: 'viewer' }),
  ...
  it('contexto de cliente não pode convidar', async () => {
    ...
    await expect(
      inviteTeamMember({ authAdmin }, customerCtx, { email: 'a@b.com', role: 'viewer' }),
    ).rejects.toBeInstanceOf(ForbiddenError);
  });
  ...
  it('não deixa conceder owner por convite (escalonamento)', async () => {
    const authAdmin = fakeAuthAdminGateway();
    await expect(
      inviteTeamMember({ authAdmin }, ctxWith('owner'), { email: 'a@b.com', role: 'owner' }),
  ...
  it('não deixa conceder papel de cliente nem papel desconhecido', async () => {
    const authAdmin = fakeAuthAdminGateway();
    await expect(
      inviteTeamMember({ authAdmin }, ctxWith('owner'), { email: 'a@b.com', role: 'customer' }),
    ).rejects.toBeInstanceOf(BusinessRuleError);
    await expect(
      inviteTeamMember({ authAdmin }, ctxWith('owner'), { email: 'a@b.com', role: 'root' }),
    ).rejects.toBeInstanceOf(BusinessRuleError);
  });
  ...
  it('e-mail já registrado propaga erro de negócio do gateway', async () => {
    ...
    await expect(
      inviteTeamMember({ authAdmin }, ctxWith('owner'), { email: 'ja@existe.com', role: 'viewer' }),
    ).rejects.toBeInstanceOf(BusinessRuleError);
  });
});
