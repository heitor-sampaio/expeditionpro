...
const CUSTOM = 'd0000000-0000-4000-8000-0000000000d4'; // saída fechada do tenant A (§3.5.1)
...
const SEED = `
  ...
  INSERT INTO itineraries (id, tenant_id, name, slug, status, kind) VALUES
    ('${ATIVO}', '${TA}', 'Ativo', 'ativo', 'active', 'catalog'),
    ('${ARQ}', '${TA}', 'Arquivado', 'arquivado', 'archived', 'catalog'),
    ('${CUSTOM}', '${TA}', 'Fechada', 'fechada', 'active', 'custom'),
    ('${ITB}', '${TB}', 'Do B', 'do-b', 'active', 'catalog');
  ...
  INSERT INTO itinerary_photos (id, tenant_id, itinerary_id, storage_path, position, is_cover) VALUES
    ...
    (gen_random_uuid(), '${TA}', '${CUSTOM}', '${TA}/fechada.webp', 0, true),
...
describe('SEC-01 · RO-01: isolamento da galeria de fotos', () => {
  ...
  it('equipe do tenant A vê todas as fotos do tenant (ativo, arquivado e fechada) e nada de B', async () => {
    const s = await TenantSession.open(TA);
    try {
      expect(await count(s, '')).toBe(4);
  ...
  it('RO-07: a apresentação abre a ficha do roteiro de catálogo, e só ela', async () => {
    /*
     * A galeria vive dentro da apresentação do roteiro, então a própria linha precisa ser
     * legível — foto sem nome não é apresentação. O que a ficha abre é descrição e
     * dificuldade, material de venda.
     *
     * O resto continua fechado, e esta é a cerca: **preço não vem junto**. O cliente lê o
     * preço da própria saída (§3.7), nunca a tabela de preços do catálogo — quanto custa
     * cada faixa em cada roteiro é decisão comercial, não vitrine. Sem este teste, uma
     * mudança futura na policy alargaria o acesso sem nada ficar vermelho.
     */
    const s = await TenantSession.openCustomer(TA, CUST_A);
    try {
      expect(await s.rows(`SELECT 1 FROM itineraries WHERE id = '${ATIVO}'`)).toHaveLength(1);
      expect(await s.rows(`SELECT 1 FROM itineraries WHERE id = '${ARQ}'`)).toHaveLength(0);
      expect(await s.rows(`SELECT 1 FROM itinerary_prices`)).toHaveLength(0);
      expect(await s.rows(`SELECT 1 FROM schedule_events`)).toHaveLength(0);
    } finally {
      await s.close();
    }
  });

  it('§3.5.1: roteiro personalizado fica fora da vitrine — nem ficha nem foto', async () => {
    /*
     * `custom` é roteiro de uma saída fechada, negociada com um grupo. O PRD o mantém
     * fora da vitrine e dos filtros públicos: quem não está naquela saída não deve nem
     * saber que ela existe. `status = 'active'` sozinho não basta como regra de catálogo.
     */
    const s = await TenantSession.openCustomer(TA, CUST_A);
    try {
      expect(await count(s, `WHERE itinerary_id = '${CUSTOM}'`)).toBe(0);
      expect(await s.rows(`SELECT 1 FROM itineraries WHERE id = '${CUSTOM}'`)).toHaveLength(0);
    } finally {
      await s.close();
    }
  });
